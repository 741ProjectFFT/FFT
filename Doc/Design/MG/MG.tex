...
\item[\refstepcounter{ucnum} \uctheucnum \label{ucInputFormat}:] Whatever the
  input format is, they are all eventually transferred to datas to be
  computed. \wss{This seems like a strange unlikely change?}
\end{description}
...
\item [\refstepcounter{mnum} \mthemnum \label{mInput}:] Input Computing Module
\item [\refstepcounter{mnum} \mthemnum \label{mOutput}:] Output  Computing Module
...
\wss{If multiple anticipated changes map to the same module, you should explain
  why.  The usual goal is one module one secret one anticipated change.}
