...
Validation (V \& V) Plan for a FFT library. \wss{Expand all acronyms
  on first usage.}
...
\wss{proof read} document is intended to be used as a reference for
...
different  FFT or IFFT functions and give proper input datas \wss{data
  is already plural} to complete a FFT or IFFT calculation. 
...
\begin{enumerate}
\item {{\large Cutest} as unit testing framework} \wss{Prior to this
    you should state your programming language.}
...
\wss{What tools are you going to use for symbolic execution?  How are
...
automatically fill with 0.\\  \wss{You get to decide the test case, so
  why not have a test case where the number of data points satisfies
  your constraint?  You can test $0$ padding separately.}
\wss{Rather than figures in the Appendix, why not include the data
  files in your repo and then give an explicit url pointer to the
  documents here?}
...
\end{gather}
$e$ means expected output. $o$ means this library's output.  \wss{What does
the index $i$ mean.  What is $n$?  I can guess, but I shouldn't have
to.  Also, use $\mathit{MSE}$}\\
If the value of MSE is below 1\% of average of input datas, then this library passed the test.

\wss{Rather than set a specific target, you can just state that the
  MSE will be provided for all tests.  Once all the data is together,
  it can be collectively judged.  1\% is an arbitrary choice on your
  part.  My guess is that the actual tests errors will be well below this.}
...
\subsubsection{Speed Comperation \wss{spell check!} Test}
\begin{enumerate}
...
\textbf {Input}: intput.txt \wss{proof read}
...
\wss{This test does not give me any useful information.  It just is
  the time for one test.  What would be more interesting is to verify
  the number of inputs in the file and find the time for each.  How
  does the execution time change as the problem size increases.  You
  could use $n$ = 100, 200, 400, 800, 1000 etc.  You could do the same
  exercise with Matlab and compare your performance to Matlab.}
...
\wss{What determines success on this test? confusing?}

\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{Mac}:Under Mac OS
      plateform \wss{spell check!}}}
...
\wss{What determines success on this test? confusing?  If you have
  make working properly, you can repeat all of your tests on any
  platform.}
...
\textbf {Output}: Truee \wss{spell check!} or False (True means the data type is right, False means the data type is wrong.)
...
\wss{This is a confusing test.  Are you talking about a test where you
  have invalid types in your input?  If so, what do you expect to
  happen.  There should likely be a requirement for an exception.  Do
  you want to make this the expected output for this test?}

\subsection{Calculation Test}

\begin{enumerate}

\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{CCC}: Complex Number Multiplication Check}}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{CCA}: complex number Addition Check}}
...
\wss{In your design I do not recall a module that hides the ADT for
  complex numbers.  Is this something you are going to implement?  If
  you are going to use an existing complex number library, you don't
  really have to test it, although you should make that fact
  explicit.}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{R2P}:Radix-2 Partition check}}
...
\item{\textbf{T-\refstepcounter{tnum}\thetnum \label{R3P}: Radix-2 Partition check}}
...
\wss{You could have used a symbolic constant for your 1\% error
  value.  If there are no constants, you don't need this section.}

\subsection{Usability Survey Questions?}

This is a section that would be appropriate for some teams.

\wss{Your project is really quite simple.  I think you should be
  thinking about some kind of usability testing, or something else
  extra, so that you can get the most out of this project.}

\wss{If you don't have a usability survey, then you shouldn't have
  this section.}
